...
# সমস্ত চ্যানেলের ওয়েব পেজ লিংকের তালিকা (যেখান থেকে টোকেন অটো-ফেচ হবে)
channels_info = [
    {"name": "T Sports HD", "page_url": "http://jatrapala.com/live-tv/t-sports.html"},
...
for ch in channels_info:
    try:
        ...
        if response.status_code == 200:
            match = re.search(r'(http[^\s\'\"<>]+tracks[^\s\'\"<>]*\.m3u8[^\s\'\"<>]*)', response.text)
            if match:
...
print(f"\nDone! Successfully fetched {success_count} channels automatically.")